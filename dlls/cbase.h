...
public:
	...
	*/
	...
	// types now go through this existing pattern instead, on NXDK only -
	// every other platform this SDK builds for has a working dynamic_cast
	// and keeps using it unchanged (see dlls/client.cpp/player.cpp). Single,
	// non-virtual inheritance the whole way down for both (verified against
	// dlls/player.h and dlls/weapons.h), so this virtual dispatch is exactly
	// behaviourally equivalent to a successful dynamic_cast, not an
	// approximation.
	...
	// NXDK only - see MyPlayerPointer() above for why. CBasePlayerItem's own
	// override stays the same NULL default; only CBasePlayerWeapon (and its
	// subclasses) return non-NULL. On every other platform CBasePlayerItem
	// keeps declaring this virtual fresh, one level down, unchanged from
	// upstream.
